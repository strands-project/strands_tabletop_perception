...
bool voxel_in_ROI(point3d p3d, geometry_msgs::Polygon poly)
{
  ...
  for (int i = 0; i < len; i++)
    {
      vsq[i] = { poly.points[i].x, poly.points[i].y };
    }

  polygon_t roi = { len, vsq };
 
	vec point = { p3d.x(), p3d.y() }; 
 
  int inside_ROI = inside(point, &roi, 1e-10);
 
  if ( inside_ROI == 1)
    {
      //ROS_INFO("YES (%f %f)", p3d.x(), p3d.y());
      return true;
    }
  //ROS_INFO("NO (%f %f)", p3d.x(), p3d.y());
  return false;
