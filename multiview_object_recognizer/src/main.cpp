...
int main (int argc, char **argv)
{
    ...
    if(!(n_->getParam ( "do_eval", do_eval)))
        do_eval = false;
    ...
    if(n_->getParam ( "extension_mode", extension_mode))
    ...
    if(!do_eval)
    {
        ...
        vis_pc_pub = n_->advertise<sensor_msgs::PointCloud2>( "multiview_recognized_objects", 0 );
    ...
    else    // do some offline evaluation (with files saved locally)
    {
        ...
        if(!n_->getParam("dataset_path", dataset_path))
            ROS_ERROR("No dataset path given (arg \"dataset_path\"). ");

        if(!n_->getParam("sequence_name", sequence_name))
