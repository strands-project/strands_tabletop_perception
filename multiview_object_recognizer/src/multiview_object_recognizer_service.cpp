...
extendFeatureMatchesRecursive ( Graph &grph,
                                ...
                                pcl::PointCloud<pcl::Normal>::Ptr pKeypointNormals)//is directed edge (so the source of calling_edge is calling vertex)
{
//    hypotheses = grph[vrtx_start].hypotheses_;
    ...
    pcl::copyPointCloud(normals_f, grph[vrtx_start].keypointIndices_, *pKeypointNormals);

    std::map<std::string, faat_pcl::rec_3d_framework::ObjectHypothesis<PointT> >::iterator it_copy_hyp;
...
//    viewer.addPointCloudNormals<pcl::PointXYZRGB,pcl::Normal>(pKeypoints, pKeypointNormals, 3, 0.03, "original");
    ...
    else    //get hypotheses from next vertex
    {
        ...
        std::cout << "Hopping to next vertex... Edges found:" << num_edges << ". Just taking the first one not being hopped." << std::endl;
...
//    pcl::PointCloud<pcl::Normal> sceneNormals;
    ...
    assert(pKeypointNormals->points.size() == grph_[vrtx].pKeypointsMultipipe_->points.size());
...
//    assert(mask_hv_sv.size() == pModels_sv->size());
    ...
    grph_final_[vrtx_final] = grph_[vrtx_final]; // shallow copy is okay here
...
//        best_edges_.push_back ( best_edge );
    ...
    //---------Extend-hypotheses-from-other-view(s)------------------------------------------
        ...
        boost::shared_ptr<std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f> > > pTransforms_mv
                (new std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f> > );
        pTransforms_mv->resize (grph_[vrtx].hypothesis_mv_.size());
        pModels_mv->resize (grph_[vrtx].hypothesis_mv_.size());
        for(size_t hyp_id=0; hyp_id < grph_[vrtx].hypothesis_mv_.size(); hyp_id++)
        {
            pTransforms_mv->at(hyp_id) = grph_[vrtx].hypothesis_mv_[hyp_id].transform_;
            pModels_mv->at(hyp_id) = grph_[vrtx].hypothesis_mv_[hyp_id].model_;
        }
        ...
        for (size_t j = 0; j < grph_[vrtx].hypothesis_mv_.size(); j++)
        {
            grph_[vrtx].hypothesis_mv_[j].verified_ = mask_hv[j];

//            if(mask_hv[j])
//            {
//                std::string id = grph_[vrtx].hypothesis_mv_[j].model_->id_;
//                std::map<std::string, faat_pcl::rec_3d_framework::ObjectHypothesis<PointT> >::iterator it_hyp_sv;
//                it_hyp_sv = grph_[vrtx].hypotheses_.find(id);
//                if (it_hyp_sv == grph_[vrtx].hypotheses_.end())
//                {
//                    std::cout << "There has not been a single keypoint detected for model " << id << std::endl;
//                }
//                else
//                {
//                    std::cout << "Augmenting hypotheses with verifed feature matches." << std::endl;
//                }
//            }
        }
